...
> ✅ **이 연구는 제공된 데이터만으로 완결할 수 있습니다. 추가 수집 없이 바로 5단계(전처리)로 진행하세요.**

**경우 B — 제공된 데이터만으로는 핵심 연결이 정말 불가능할 때 (A가 불가능한 경우에만):**
연구 주제 제안 직후 아래 형식으로 외부 데이터를 즉시 안내하세요:
> ⚠️ **지금 바로 추가 데이터를 수집하세요 (5단계 진행 전 필수):**
> - 필요한 이유: [왜 이 데이터 없이는 연구가 불완전한지 한 문장으로]
> - 필요한 데이터 이름: [구체적인 명칭]
> - 추천 출처: [공공데이터포털(data.go.kr) / 국가통계포털(kosis.kr) / 서울열린데이터광장(data.seoul.go.kr) 중 해당 URL 명시]
> - 검색 키워드: ['키워드1', '키워드2']
> - 파일 형식: CSV 또는 Excel
...
5. **5단계 자동 매칭을 위해**, 답변 시 위에서 제공된 **실제 파일명**(.csv 포함)을 문맥 속에서 최소 한 번 이상 정확하게 언급해 주세요. (예: "이 연구를 위해 '서울교통공사_혼잡도.csv' 데이터를 활용합니다.")`;
