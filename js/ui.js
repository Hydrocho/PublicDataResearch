...
/**
 * Shared: Full 4단계 Problem Definition UI.
 * @param {string} containerId  - ID of the element to render into
 * @param {Function} getDatasetsFn - async () => { data: [...], error }
 * @param {Function|null} saveRecordFn - async ({ answer, opinion }) => void  (null = teacher mode, no save)
 * @param {Function} goToManagementFn - called when "데이터 관리로 가기" is clicked
 */
/**
 * 5단계: 데이터 전처리 — 학생/교사 공유 렌더 함수
 * @param {string} containerId
 * @param {object} opts
 *   getOwnLogsFn           async () => {data, error}  — own step3 activity logs
 *   getTeamLogsFn          async () => {data}  or null
 *   getDatasetsFn          async () => {data, error}  — datasets for colab prompt
 *   getSelectedId          () => string|null
 *   setSelectedIdAndRerender  (id|null) => void
 *   onDelete               async (id) => void  or null (hides delete buttons)
 *   onGoToStep4            () => void  or null
 *   isTeacherMode          bool
 */
...
export async function renderProblemDefinitionView(containerId, getDatasetsFn, saveRecordFn, goToManagementFn) {
    ...
    // 교사 테스트 저장 버튼 — AI 답변을 localStorage에 저장하여 5단계에서 사용
    if (isTeacher) {
